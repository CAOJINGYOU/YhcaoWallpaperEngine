...
void CUnsplashNet::execute(QString strUrl)
{
    ...
    QUrl url(strUrl+"/"+QString("%1x%2").arg(width).arg(height));
...
{
    QString fileName = QStandardPaths:: writableLocation(QStandardPaths::PicturesLocation)+QDir::separator()+"Unsplash"+QDir::separator();
    QDir dir(fileName);
    if(!dir.exists())
    {
        dir.mkpath(fileName);
